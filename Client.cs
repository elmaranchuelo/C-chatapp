        ...
        private void Log(string msg = "") 
        {
            if (!exit)
            {
                logTextBox.Invoke((MethodInvoker)delegate
                {
                    if (msg.Length > 0)
                    {
                        logTextBox.AppendText(string.Format("[ {0} ] {1}{2}", DateTime.Now.ToString("MMM dd yyyy HH:mm"), msg, Environment.NewLine));
                    }
                    else
                    {
                        logTextBox.Clear();
        ...
        private void Read(IAsyncResult result)
        {
            ...
            if (bytes > 0)
            {
                ...
                try
                {
                    ...
                    {
                        CryptographicAlgorithm algo = new CryptographicAlgorithm();
                        string key = keyTextBox.Text;
                        string DecryptText = algo.Decrypt(obj.data.ToString(), key);
                        string msg = string.Format("{0}: {1}", obj.username, DecryptText);
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    string cipherText = algo.Encrypt(msg, key);
                    sendTextBox.Clear();
